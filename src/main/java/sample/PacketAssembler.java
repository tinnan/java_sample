...
public class PacketAssembler {
    public static void main(String args[]) {
        ...
        while (sc.hasNextLine()) {
            ...
            packets.add(new PacketAssembler.Packet(msg));
        }
        sc.close();
    ...
    private static class Packet implements Comparable<Packet> {
        ...
        public Packet(String msg) {
            // sry, too lazy to do math.
            this.messageId_ = Integer.parseInt(msg.substring(0, 8).trim());
            this.packetId_ = Integer.parseInt(msg.substring(8, 8 + 4).trim());
            this.totalPacket_ = Integer.parseInt(msg.substring(8 + 4, 8 + 4 + 4).trim());
            this.message_ = msg.substring(8 + 4 + 4);
        }
        
        @Override
        public int compareTo(Packet p) {
            int ret = this.messageId_ - p.messageId_;
            if (ret == 0) {
                ret = this.packetId_ - p.packetId_;
            }
            return ret;
        }
        ...
        public String toString() {
            return String.format("%-8d%-4d%-4d%s", messageId_, packetId_, totalPacket_, message_);
        }
    }
}
